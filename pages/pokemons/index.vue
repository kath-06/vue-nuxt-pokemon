<template>
  <div class="p-4">
    <VnPageHeader @pokeSearch="handleSearch" text="Pokemon List" />
...
const getPokemon = async (offsetData: any) => {
  ...
  const { data, refresh }: any = await useFetch(
    `/api/pokemon?offset=${offsetData}&limit=${limit.value}`,
    { query: { type: "getList" } }
  );
  if (data.value === null) {
    await refresh();
...
// const emitPokeSearch = (value: any) => {
//   emit("pokeSearch", value);
//   console.log(value);
// };
